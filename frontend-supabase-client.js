// ============================================
// HDIMS Frontend Supabase Client
// ============================================
...
export const rpc = {
  ...
  submitPerformanceData: async (params) => {
    const { data, error } = await supabase.rpc(config.rpc.submitPerformanceData, params);
    return { data, error };
  },

  // Submit scheme data
  submitSchemeData: async (params) => {
    const { data, error } = await supabase.rpc(config.rpc.submitSchemeData, params);
    return { data, error };
  },

  // Review submission
  reviewSubmission: async (params) => {
    const { data, error } = await supabase.rpc(config.rpc.reviewSubmission, params);
    return { data, error };
  },

  // Get facility dashboard
  getFacilityDashboard: async (facilityId, startDate, endDate) => {
    const { data, error } = await supabase.rpc(config.rpc.getFacilityDashboard, {
  ...
  getDistrictDashboard: async (districtId) => {
    const { data, error } = await supabase.rpc(config.rpc.getDistrictDashboard, {
  ...
  getStateDashboard: async (stateId) => {
    const { data, error } = await supabase.rpc(config.rpc.getStateDashboard, {
      p_state_id: stateId,
    });
    return { data, error };
  },
  ...
  getPerformanceTrends: async (facilityId, metricKey, months) => {
    const { data, error } = await supabase.rpc(config.rpc.getPerformanceTrends, {
      p_facility_id: facilityId,
  ...
  getSuperadminOverview: async () => {
    const { data, error } = await supabase.rpc(config.rpc.getSuperadminOverview);
    return { data, error };
  },
};
...
export const edgeFunctions = {
  ...
  submitPerformance: async (data) => {
    const response = await fetch(config.functions.submitPerformance, {
      method: 'POST',
      headers: {
  ...
  submitScheme: async (data) => {
    const response = await fetch(config.functions.submitScheme, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`,
        'Content-Type': 'application/json',
  ...
  reviewSubmission: async (data) => {
    const response = await fetch(config.functions.reviewSubmission, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`,
        'Content-Type': 'application/json',
  ...
  getDistrictAnalytics: async (districtId, options = {}) => {
    const url = new URL(config.functions.analyticsDistrict);
  ...
  getStateAnalytics: async (stateId, options = {}) => {
    const url = new URL(config.functions.analyticsState);
  ...
  uploadDocument: async (file, metadata) => {
    ...
    const response = await fetch(config.functions.uploadDocument, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`,
      },
